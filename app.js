(function () {
  ...
  // A product at or below this remaining quantity is flagged "ใกล้หมด" (low
  // stock) in the warehouse tab's color-coded alerts (green/yellow/red).
  // 0 or below is always "หมดสต๊อก" (out of stock) regardless of this value.
  ...
  // Note: the selected department used to be restored from localStorage here
  // on every page load, even across a fresh login — which is exactly why it
  // (and the search boxes) kept "sticking" from whoever used the app last.
  // resetTransientState_() below now clears it on every successful login
  // instead, so each person starts from a clean screen.
  ...
  // Clears every filter/search/selection field back to its default so each
  // fresh login starts on a clean screen — no leftover search text, category,
  // or department selected by whoever used the app before.
  function resetTransientState_() {
    state.tab = 'transact';
    state.mode = 'withdraw';
    state.deptCode = '';
    ...
    state.reportFrom = '';
    state.reportTo = '';
    state.reportDeptFilter = '';
    state.expandedDept = null;
    state.whSearch = '';
    state.whStockFilter = 'all';
    ...
    state.stockCountCategory = 'ทั้งหมด';
    state.stockCountSearch = '';
    state.stockCountNotes = '';
    state.stockCountExpanded = null;
    state.userConfirmDelete = null;
    try { localStorage.removeItem('req_app_dept'); } catch (e) {}
  }
  ...
  // ---------- Product admin CRUD ----------
  // Note: the product code is intentionally NOT editable here — it's the
  // key every past requisition, stock-count record and report references,
  // so changing it after the fact would orphan that history. To fix a
  // wrong code, delete the product and add it again with the right one.
  async function saveProductEdit(code, data) {
    ...
    try {
      if (apiAvailable) {
        await apiUpdate('products', code, payload);
        await loadProducts_();
      } else {
  ...
  function render() {
    ...
    // Header + subtitle + tab nav are all wrapped together and pinned as one
    // sticky unit (see .sticky-head in the CSS) so the menu never scrolls
    // out of view — however tall the header gets (it wraps on narrow
    // screens), the nav directly below it stays pinned along with it.
    const stickyHead = el('div', { class: 'sticky-head' });
    ...
    stickyHead.appendChild(el('p', { class: 'subtitle' }, [txt('Bangkok Hospital Samui')]));
  ...
  function renderProductList() {
    ...
    list.forEach(p => {
      const stockClass = p.stock <= 0 ? 'zero' : (p.stock <= LOW_STOCK_THRESHOLD ? 'low' : 'ok');
  ...
  // ---------- Warehouse tab ----------
  // Summary counts behind the warehouse tab's green/yellow/red alert tiles.
  function stockAlertCounts_() {
  ...
  function renderStockAlertTiles_() {
    const c = stockAlertCounts_();
    const tiles = [
      { key: 'all', cls: 'neutral', icon: '📦', label: 'สินค้าทั้งหมด', count: c.total },
      { key: 'ok', cls: 'ok', icon: '🟢', label: 'ปกติ', count: c.ok },
      { key: 'low', cls: 'low', icon: '🟡', label: 'ใกล้หมด', count: c.low },
      { key: 'zero', cls: 'zero', icon: '🔴', label: 'หมดสต๊อก', count: c.zero }
    ];
    return el('div', { class: 'stock-alert-row' }, tiles.map(t =>
      el('button', {
        class: 'stock-tile ' + t.cls + (state.whStockFilter === t.key ? ' active' : ''),
        onclick: () => { state.whStockFilter = t.key; renderWarehouseList(); refreshStockAlertTiles_(); }
      }, [
        el('div', { class: 'stock-tile-num' }, [txt(String(t.count))]),
        el('div', { class: 'stock-tile-lbl' }, [txt(t.icon + ' ' + t.label)])
      ])
    ));
  }
  // Re-renders just the tile row so clicking a tile updates its own
  // active/highlighted state without a full-page rebuild.
  function refreshStockAlertTiles_() {
    const wrap = document.getElementById('stockAlertWrap');
    if (!wrap) return;
    wrap.innerHTML = '';
    wrap.appendChild(renderStockAlertTiles_());
  }

  function renderWarehouse(app) {
    const card = el('div', { class: 'card' });

    const alertWrap = el('div', { id: 'stockAlertWrap' }, [renderStockAlertTiles_()]);
    card.appendChild(alertWrap);
    card.appendChild(el('div', { style: 'font-size:.74rem;color:var(--text-dim);margin:2px 0 14px;' }, [
      txt(`🟡 ใกล้หมด = คงเหลือ ${LOW_STOCK_THRESHOLD} หน่วยหรือน้อยกว่า • 🔴 หมดสต๊อก = คงเหลือ 0 — คลิกที่การ์ดด้านบนเพื่อกรองรายการด้านล่าง`)
    ]));
  ...
  function renderWarehouseList() {
    ...
    if (q) list = list.filter(p => p.name.toLowerCase().includes(q) || p.code.includes(q));
    if (state.whStockFilter === 'zero') list = list.filter(p => p.stock <= 0);
    else if (state.whStockFilter === 'low') list = list.filter(p => p.stock > 0 && p.stock <= LOW_STOCK_THRESHOLD);
    else if (state.whStockFilter === 'ok') list = list.filter(p => p.stock > LOW_STOCK_THRESHOLD);
    if (!list.length) {
      const noteText = state.whStockFilter === 'all' ? 'ไม่พบสินค้า' : 'ไม่พบสินค้าในกลุ่มนี้';
      wrap.appendChild(el('div', { class: 'empty-note' }, [txt(noteText)]));
      return;
    }
    ...
    list.forEach(p => {
      ...
      if (state.whEditCode === p.code) {
        ...
        row.appendChild(el('div', { style: 'font-size:.78rem;color:var(--text-dim);margin-bottom:6px;' }, [
          txt('รหัสสินค้า: '), el('b', { style: 'color:var(--text);' }, [txt(p.code)]), txt(' (แก้ไขรหัสไม่ได้ — เพื่อไม่ให้ประวัติการเบิก/รายงานเก่าอ้างอิงผิด ถ้ารหัสผิดให้ลบแล้วเพิ่มสินค้าใหม่)')
        ]));
        const grid = el('div', { class: 'edit-grid' });
        ...
        [nameIn, catIn, unitIn, priceIn, stockIn].forEach(i => grid.appendChild(i));
        ...
        const btnRow = el('div', { style: 'display:flex;gap:8px;' }, [
          el('button', { class: 'btn success sm', onclick: () => saveProductEdit(p.code, { name: nameIn.value, category: catIn.value, unit: unitIn.value, price: priceIn.value, stock: stockIn.value }) }, [txt('บันทึก')]),
